...
function _generateParameters(config, context) {
    ...
    params.SelectQueryParam = {
        ...
        required: false,
        type: 'string',
        description: 'This helps with what to return for the user and is "," delimited'
    };
    ...
    params.IDPathParam = {
        ...
        required: true,
        type: 'string',
        description: 'This is to select a particular object'
    };
            ...
            if (_.find(tags, {identity: identity})) {
                controller = _.merge(controller, {
                    findOne: {
                        ...
                        description: 'This is used to get a single {identity} with the supplied id'
                    },
                    find: {
                        http_method: 'get',
                        path: prefix + identity,
                        summary: 'List {identity}',
                        description: 'This is used to retrieve a list of {identity}'
                    },
                    create: {
                        http_method: 'post',
                        path: prefix + identity,
                        summary: 'Create {identity}',
                        description: 'This is used to create new {identity}'
                    },
                    update: {
                        http_method: 'put',
                        path: prefix + identity + '/{id}',
                    ...
                    destroy: {
                        ...
                        description: 'This is for deleting a specified {identity} by id'
                    }
                });
            }
        }
        ...
        _.forEach(routes, function (route) {
          ...
          if(!route.custom){ // if not from routes.js use the default custom blueprint maps
            ...
            var path = {
                ...
                operationId: crypto.createHash('md5').update(route.http_method + route.path).digest('hex'),
